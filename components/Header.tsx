      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-2">
          {["Home", "About", "Portfolio", "Contact"].map((item) => (
            <button
              key={item}
              className="px-4 py-2 text-lg font-semibold transition-all duration-300 hover:text-red-700 hover:scale-125 rounded-lg "
            >
              {item}
            </button>
          ))}
        ...
        {/* CTA Button - Fixed Width (same as logo) */}
        <div className="w-32 flex justify-end">
          <a
            href="#contact"
            className="hidden md:block px-6 py-2.5 rounded-lg font-bold shadow-lg bg-primary text-white transition-all duration-300 hover:bg-third hover:scale-105"
          >
            Hire Me
          </a>
